...
const Activities = () => {
  ...
  const filteredActivities = activities?.filter((activity) => {
    ...
    const leadName = leads.find(l => l._id === activity.lead_id)?.name.toLowerCase() || '';
    const leadCompany = leads.find(l => l._id === activity.lead_id)?.company.toLowerCase() || '';
  ...
  return (
    <div className="container mx-auto p-6">
      {/* Page Title */}
      <h1 className="text-3xl font-bold text-white mb-6">Activity Management</h1>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between mb-6 gap-4">
        ...
        <Dialog open={isAddActivityDialogOpen} onOpenChange={setIsAddActivityDialogOpen}>
          <Button onClick={() => setIsAddActivityDialogOpen(true)} className="w-full md:w-auto">
      ...
      {/* Activities Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"> {/* <--- Updated grid layout */}
        {filteredActivities.length > 0 ? (
          filteredActivities.map((activity) => {
            const associatedLead = leads.find((lead) => lead._id === activity.lead_id);
          ...
          <div className="col-span-full text-center py-12">
            <p className="text-gray-400 text-lg">
              {searchTerm ? 'No activities match your search.' : 'No activities found. Create your first activity!'}
            </p>
          </div>
        )}
      </div>
