...
const Leads = () => {
  ...
  const { mutate: createDummyLead, isLoading: isCreatingDummy } = useCreateLead();
  ...
  const handleManualAdd = () => {
    ...
    createDummyLead(dummyLead, {
      ...
      onError: (err) => toast.error(`Failed to add dummy lead: ${err.message || 'Unknown error'}`),
    });
  };
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 min-h-screen bg-transparent text-white"> {/* Removed gradient from here, now in Layout */}
      <h1 className="text-4xl font-bold mb-6 text-white text-center">Lead Management</h1>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-6">
        ...
        <div className="flex gap-2">
          ...
          >
            <PlusCircle className="h-4 w-4" /> {isCreatingDummy ? 'Adding...' : 'Add Dummy Lead'}
          </Button>
          <Button
      ...
      <AddLeadDialog isOpen={isAddLeadDialogOpen} onOpenChange={setIsAddLeadDialogOpen} />
      ...
      {isLoading ? (
        ...
        <p className="text-red-400 text-center">Error loading leads: {error.message}</p>
      ) : leads && leads.length === 0 ? (
        <p className="text-center text-gray-300">No leads found. Add a new lead to get started!</p>
      ) : (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {leads.map((lead) => (
            <LeadCard 
