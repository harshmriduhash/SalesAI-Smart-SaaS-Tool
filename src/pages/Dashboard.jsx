...
const GlassmorphismCard = ({ children, className = "" }) => (
  <Card
    className={`bg-white/10 backdrop-blur-lg border border-white/20 shadow-xl ${className}`}
  >
    {children}
  </Card>
);
...
const Dashboard = () => {
  ...
  const totalValue =
    leads?.reduce((sum, lead) => sum + (lead.estimated_value || 0), 0) || 0;

  // Simple conversion rate (Closed Won / Total Leads)
  const conversionRate =
    totalLeads > 0 ? ((closedWonLeads / totalLeads) * 100).toFixed(1) : 0;
  ...
  const getConversionTrendData = (leads) => {
    ...
    return sortedMonths.map((monthYear) => {
      ...
      const rate =
        monthStats.leads > 0
          ? (monthStats.closedWon / monthStats.leads) * 100
          : 0;
      return {
        name: monthYear,
        "Conversion Rate": parseFloat(rate.toFixed(1)),
      };
    });
  };
  ...
  if (isError) {
    return (
      <div className="flex justify-center items-center min-h-screen bg-transparent text-red-400 text-lg">
        {" "}
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 bg-transparent text-white space-y-8">
      {" "}
      {/* Removed gradient from here */}
