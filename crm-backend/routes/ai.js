...
router.post('/generate-email', async (req, res) => { // Removed 'auth' middleware here, it's applied in server.js
  if (!AI_ENABLED) {
...
router.post('/analyze-lead/:id', async (req, res) => { // Removed 'auth' middleware here
  if (!AI_ENABLED) {
    // Provide consistent mock data for AI insights and conversation prep
    return res.json({
      success: true,
      data: {
        ai_insights: "Based on historical data and market trends, this lead shows strong potential due to high engagement with past marketing campaigns and a clear need for our core product. Recommend focusing on a value proposition that highlights efficiency gains.",
        conversation_prep: "Key discussion points: 1. Acknowledge their industry's current challenges. 2. Present case study of similar company. 3. Propose a personalized solution. 4. Address potential budget concerns upfront. 5. Next steps: schedule a follow-up demo."
      }
    });
  }
...
Last Contact: ${lead.last_contact_date ? lead.last_contact_date.toISOString() : 'N/A'}
Next Follow-up: ${lead.next_follow_up ? lead.next_follow_up.toISOString() : 'N/A'}
Pain Points: ${lead.pain_points && lead.pain_points.length > 0 ? lead.pain_points.join(', ') : 'N/A'}
Interests: ${lead.interests && lead.interests.length > 0 ? lead.interests.join(', ') : 'N/A'}
...
router.post('/score-lead', async (req, res) => { // Removed 'auth' middleware here
  if (!AI_ENABLED) {
    return res.json({
      success: true,
